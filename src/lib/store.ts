  ...
  persist(
    (set, get) => ({
      ...
      companySettings: {
        ...
        logo: '',
        primaryColor: '20 50% 47%', // Default Sienna
        currency: 'USD',
      },

      login: (email, role, password) => {
        const user = get().users.find(u => u.email.toLowerCase() === email.toLowerCase() && u.role === role);
      ...
      signUp: (name, email, role, password) => set((state) => {
        const newUser = { id: Math.random().toString(), name, email, role, password };
        return {
      ...
      addUser: (u) => set((state) => ({
        users: [...state.users, { ...u, id: Math.random().toString() }]
      })),
      ...
      addCustomer: (c) => set((state) => ({
        customers: [...state.customers, { ...c, id: Math.random().toString(), purchaseHistory: [] }]
      })),
      ...
      addProduct: (p) => set((state) => ({
        products: [...state.products, { ...p, id: Math.random().toString() }]
      })),
    ...
    {
      name: 'cinnamon-link-pro-storage',
    }
  )
);
